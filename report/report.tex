...
perform all sorts of tricks\ref{safe-abi}, depending on the operating system and
...
% Hoe haal je signals op? Hoe stuur je signals door naar tracees?
% ptrace(PTRACE_SYSCALL, data=SIGNAL)

% Rare ptrace signals (PTRACE_EVENT_FORK met PTRACE_O_TRACEFORK etc)
...
% Eerste SIGSTOP negeren?

\subsection{Clone on x86}

% Args switched... Waarom? Misschien wel leuk om uit te zoeken.
...
% A solution to this problem would be support from the Linux kernel; a way to
% get the mode of the current system call, the best way would be to deliver this
% information in the system call trap signal when using the
% \textit{PTRACE\_O\_TRACESYSGOOD} option. However, this requires kernel changes
% and is therefore not an immediate solution.
%
% Another solution would be to analyse the assembly instruction that was used to
% perform the system call. On x86 a system call is performed with the
% \textit{int 0x80} instruction whereas a x86-64 instruction is performed with
% the \textit{syscall} instruction. To prevent a race condition similar to the
% one presented and solved in \cite{Noordende_asecure}, the memory pages should
% be either writable, or readeable and executable. This way we can assure that
% the prisoner does not quickly change the instruction in another thread -
% before we read but after the initial system call has been made. Another 
% advantage of this approach it is possible to ``cache'' the system call
% instruction. Because it is not possible to change the instruction without
% changing the permissions of the memory. It is possible to temporarily give
% write access to the memory and then update the system call instruction cache.
