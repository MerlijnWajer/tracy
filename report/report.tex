...
% , as described in Section \ref{future-threaded-tracees}.

% O: execve() + not main thread? (wrt exit events, zie ook PTRACE_O_TRACEEXIT)
...
% XXX: Invalidate de context switch niet al de instructies?
...
%
%To properly trace a 32 bit process we need to be able to identify the
%architecture (instruction set) of the process.
% /usr/include/elf.h
% LD_SHOW_AUXV=1
% /proc/<pid>/auxv
% Kan je ze zelf aanpassen (als programma)?
...
% a brief overview of the current API can be found in Appendix \ref{tracy-api}.
The API provides
