...
\lstset{
    ...
    caption={Intel x86 Assembly that performs a system call}
}
% TODO: Needs more asm + explanation
...
As stated on Section \ref{secure-abi}, Tracy is not yet able to safely trace
processes, the solution presented in Section \ref{secure-abi} should however
eliminate all currently known ways to ``escape'' from Tracy. Implementation the
aforementioned solution will also require the ability to ``understand'' multiple
ABIs as presented in Section \ref{diff-abi-trace}. % Nog iets meer over hoe
% Tracy informatie gaat geven over de ABI? (event->abi ofzo?)
We believe that upon completion of features ``Secure ABI Mixing'' (Section
\ref{secure-abi}) and
``Threaded Tracees'' (Section \ref{future-threaded-tracees}),
Tracy will be fit for creating programs which require safe tracing,
such as a \textit{jail}. % valt een beetje uit de lucht
...
architecture. This makes it impossible to inject a system call such as
...
The fact that the Tracy API is not completely architecture and operating system
...
the tracer performs. As noted previously (Section \ref{ptrace-perf-problems},
...
solution but we see no short-term solution to the performance issues. However,
the performance issues should be neglectable when tracing a program that doesn't
perform a lot of IO. \\
