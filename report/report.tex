...
\item Safely tracing a program. A program should not be able execute code that
    is not detected (properly). % VAAG XXX
...
%TODO Uitleggen waarom statically linked uitmaakt en wat het is.
...
an application of Tracy called Soxy: a transparent SOCKS5 proxifier. % XXX: SOCKS5
Chapter 5 presents an overview of features that Tracy currently lacks and
features we would like to see implemented in Tracy. We end the paper with recap
of the paper, with a focus on goals set by the Tracy research project.
% TODO: We sluiten het paper af met een samenvatting/overzicht van wat we nu
% kunnen, wat er nog moet gebeuren e.d. (Discussie)
...
suspended. This may seem counter intuitive, but that's the way threads are
implemented on Linux. % TODO: Hoe is het op BSD?
...
spare, file descriptor tables and file system attributes.

To reliably trace opened files and sockets, programmers need to know what
threads (or more general: processes) share file descriptor tables and memory. To
complicate matters, the \textbf{unshare} system call allows a process to
seperate its file descriptor table as well as its file system attributes.
% XXX: Zeggen dat Tracy hier nog niets mee doet? (Ja, moet het dan ook in future
% work? Ja)
...
% TODO: integrate https://en.wikipedia.org/wiki/W%5EX (W^X)
...
sure the cpu pipeline is flushed).
...
% Key areas van 1.1 meenemen; in hoeverre kan je nu safe tracen, in hoeverre
% is het nu echt cross platform, in hoeverre is de API nu simpel om te
% gebruiken, etc.
% Caveats gebruiken  ^ hierbij. Bijv ARM r0 en return_cod as poss.
%
% Soxy caveat (socketcall vs socket direct)
